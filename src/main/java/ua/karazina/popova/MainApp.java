...
import ua.karazina.popova.service.TextLoader;
import ua.karazina.popova.service.TextProcessor;
...
import static ua.karazina.popova.service.TextProcessor.mapMessagesToWords;
import static ua.karazina.popova.service.TextStatistic.calculateAverageMessageLength;
import static ua.karazina.popova.service.TextStatistic.calculateAverageWordLength;
import static ua.karazina.popova.service.TextStatistic.calculateMessagesLengthCount;
import static ua.karazina.popova.service.TextStatistic.calculateWordsFrequency;
import static ua.karazina.popova.service.TextStatistic.calculateWordsFrequencyMap;
import static ua.karazina.popova.service.TextStatistic.calculateWordsLengthCount;
...
public class MainApp extends Application {
    ...
    private void mainLogic() throws IOException {
        lab1();
        lab2();
    } //яблоко яблочный яблочное яблочные яблочная ябло

    private void lab1() throws IOException {
        ...
        calculateButton.setOnMouseClicked(event -> {
            ...
            try {
                if(!filePathField.getText().isEmpty()) {
                    data = TextLoader.loadData(new File(filePathField.getText()));
                }else{
                    data = TextLoader.loadData(new File("sms-spam-corpus.csv"));
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
            ...
            int countHamWordsFromSource = mapMessagesToWords(hamMessages).size();
            int countSpamWordsFromSource = mapMessagesToWords(spamMessages).size();

            double PhamInSource = (double) countHamWordsFromSource / (countHamWordsFromSource + countSpamWordsFromSource);
            double PspamInSource = (double) countSpamWordsFromSource / (countHamWordsFromSource + countSpamWordsFromSource);

            List<String> wordsOfInputMessage = mapMessagesToWords(TextProcessor.processMessage(textArea.getText()));
            ...
            long countOfAbsentWordsHamInInputWords = wordsOfInputMessage.stream()
                    .filter(key -> !hamsFrequency.containsKey(key))
                    .count();
            long countOfAbsentWordsSpamInInputWords = wordsOfInputMessage.stream()
                    .filter(key -> !spamsFrequency.containsKey(key))
                    .count();

            double PbodyHam = wordsOfInputMessage.stream()
                    .map(hamsFrequency::get)
                    .filter(Objects::nonNull)
                    .map(value -> value + 1)
                    .mapToDouble(value -> (double) value / (countHamWordsFromSource + countOfAbsentWordsHamInInputWords))
                    .reduce(1, (a, b) -> a * b);
            double PbodySpam = wordsOfInputMessage.stream()
                    .map(spamsFrequency::get)
                    .filter(Objects::nonNull)
                    .map(value -> value + 1)
                    .mapToDouble(value -> (double) value / (countSpamWordsFromSource + countOfAbsentWordsSpamInInputWords))
                    .reduce(1, (a, b) -> a * b);

            double hamResult = PhamInSource * PbodyHam;
            double spamResult = PspamInSource * PbodySpam;
